...
# CRITICAL: Lock global para serializar lançamentos do browser em batch
# Evita múltiplos browsers iniciando simultaneamente e saturando CPU/memória da VM
# IMPORTANTE: threading.Lock (não asyncio.Lock) porque batch roda em threads separadas
_browser_launch_lock = threading.Lock()

# ✅ OPÇÃO B: Global simples para execução sequencial (um processo por vez)
# Setado no INÍCIO de execute_rpa()/run_elaw_login_sync(), limpo no finally
# ZERO race conditions porque:
# 1. Batch executa processos SEQUENCIALMENTE (um por vez)
# 2. Mutex _execute_rpa_lock previne web + batch simultâneos
_current_process_id: Optional[int] = None
_execute_rpa_lock = threading.Lock()  # Previne execute_rpa() concorrente (web + batch)
...
NAV_TIMEOUT_MS = int(os.getenv("RPA_NAV_TIMEOUT_MS", "60000"))  # 60s (tolerante a VPN com latência de 40-45s)
...
def _get_screenshot_path(filename: str, process_id: Optional[int] = None) -> Path:
    """
    🔧 2025-11-21: Opção B - process_id opcional com fallback global + graceful degradation
    Retorna caminho único de screenshot por processo
    
    Args:
        filename: Nome base do arquivo (ex: 'elaw_flow_error.png')
        process_id: ID do processo (opcional - usa _current_process_id se None)
    ...
    pid = process_id if process_id is not None else _current_process_id
    if pid is None:
        # ⚠️ GRACEFUL DEGRADATION: Retornar path genérico em vez de crashar
        # Isso permite que error handlers e código legado continuem funcionando
        log(f"[SCREENSHOT][WARN] _get_screenshot_path sem process_id - usando path genérico (filename={filename})")
        return SCREENSHOT_DIR / filename  # Fallback genérico
...
def update_status(step: str, message: str, status: str = "running", data: dict = None, process_id: Optional[int] = None):
    ...
    2025-11-21: Opção B - process_id opcional com fallback global
    ...
    pid = process_id if process_id is not None else _current_process_id
    if pid is None:
        log(f"[STATUS][ERROR] update_status chamado sem process_id e _current_process_id=None (step={step}, message={message})")
        return
    ...
    if STATUS_ENABLED and flask_app:
        try:
            ...
            log(f"[STATUS][#{process_id}][WARN] Erro ao atualizar status: {e}")
...
def update_field_status(field_key: str, field_label: str, value: Any = None, process_id: Optional[int] = None):
    ...
    pid = process_id if process_id is not None else _current_process_id
    if pid is None:
        log(f"[STATUS][ERROR] update_field_status chamado sem process_id e _current_process_id=None para campo {field_key}")
        return
...
async def launch_browser():
    async with async_playwright() as p:
        ...
        # Usar Chromium do Nix (mais novo, requer novo modo headless)
        import subprocess
        try:
            executable_path = subprocess.check_output(['which', 'chromium'], text=True).strip()
            log(f"[BROWSER] Usando Chromium do Nix: {executable_path}")
            # Chromium 138+ requer novo modo headless
            if HEADLESS:
                args.append("--headless")  # Novo modo headless (não mais --headless=old)
        except Exception as e:
            executable_path = None
            log(f"[BROWSER][WARN] Chromium do Nix não encontrado, usando Playwright padrão: {e}")
...
async def login_elaw(page, user: str, password: str, url: str) -> bool:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            await email_loc.wait_for(state="attached", timeout=10000)
...
async def open_menu_processos(page):
    ...
    await goto_with_retries(page, target, attempts=1, nav_timeout_ms=7000)
        ...
        for selector in pivot_selectors:
            try:
                ...
                await page.wait_for_selector(
                    selector,
                    state="visible",
                    timeout=10000  # 10s - tolerante a AJAX lento
                )
        ...
        with app.app_context():  # type: ignore
            if process_id:
                ...
                if p:
                    ...
                    log(f"[DATA] DB (id={process_id}) OK - CNJ: {data.get('cnj', 'N/A')}, Parte Adversa: {data.get('parte_adversa_nome', 'N/A')}, PDF: {data.get('pdf_filename', 'N/A')}, Reclamadas: {len(data.get('reclamadas', []))}, Pedidos: {len(data.get('pedidos', []))}")
                    return data
            p = (
...
async def fill_new_process_form(page, data: Dict[str, Any], process_id: int):  # 2025-11-21: process_id OBRIGATÓRIO
    ...
    # 🔧 DEBUGGING: Logar process_id e dados CNJ do banco
    # process_id já foi recebido como parâmetro (Opção B - não precisa mais do global)
    ...
    # 12) Assunto (AreaProcessoId) - COM FALLBACK COMPLETO
    try:
        ...
        # Fallback final: Reclamação Trabalhista ou primeira opção
        if not wanted and assunto_opts:
            wanted = _best_match(assunto_opts, "Reclamação Trabalhista", threshold=10) or assunto_opts[0]
            log(f"[Assunto] Usando fallback final: {wanted}")
        
        if wanted:
            _must(
                await set_select_fuzzy_any(page, "AreaProcessoId", wanted, fallbacks=assunto_opts[:8] if assunto_opts else None),
                "Assunto",
            )
            await _settle(page, "select:assunto")
            update_field_status("assunto", "Assunto", wanted)
    except Exception as e:
        log(f"[Assunto][WARN] {e}")
    ...
    # 16) Tipo de Ação (⭐ garante execução)
    try:
        ...
        if wanted:
            _must(
                await set_select_fuzzy_any(page, "TipoAcaoId", wanted, fallbacks=tp_opts[:10] if tp_opts else None),
                "Tipo de Ação",
            )
            await _settle(page, "select:tipo_acao")
            update_field_status("tipo_acao", "Tipo de Ação", wanted)
        else:
            log("[TipoAção][WARN] não foi possível montar a lista/seleção")
    except Exception as e:
        log(f"[TipoAção][WARN] {e}")

    # 16.b) Objeto/Classe correlata (ObjetoId/Objeto/ClasseId)
    try:
        ...
        if alvo_id:
            ...
            _must(
                await set_select_fuzzy_any(page, alvo_id, wanted, fallbacks=opts[:10] if opts else None),
                f"Objeto ({alvo_id})",
            )
...
async def ensure_elaw_detail_url_via_relatorio(page, process_id: int, data: dict) -> bool:
    ...
    try:
        ...
        for tentativa_numero in numeros_para_tentar:
            ...
            try:
                await page.wait_for_selector('#processoRelatorioList', state='visible', timeout=10000)
            except Exception:
...
# =========================
# PEDIDOS - Adicionar novos pedidos (múltiplos)
# =========================
async def handle_novo_pedido(page, data: dict, process_id: int) -> bool:
    """
    Adiciona MÚLTIPLOS pedidos na aba Pedidos.
    
    Fluxo para cada pedido:
    1. Clica na aba "Pedidos" na sidebar
    2. Abre dropdown "Ações"
    3. Clica em "Novo Pedido"
    4. No modal, seleciona o Tipo do Pedido
    5. Clica no campo "Valor Pedido" para preencher 0,00
    6. Clica em "Salvar"
    7. Repete para cada tipo de pedido encontrado
    8. Captura screenshot final
    ...
    """
    log("[PEDIDOS][RPA] Iniciando adição de pedidos...")
    update_status("pedidos_inicio", "Abrindo aba Pedidos...", process_id=process_id)
    
    try:
        # 1. Clicar na aba "Pedidos" na sidebar
        ...
        try:
            ...
            # Verificar se a aba ficou visível
            ...
            if not box_visible:
                ...
                await page.evaluate("""() => {
                    // Remover classe 'active' das outras abas
                    ...
                    if (pedidosLink) {
                        pedidosLink.parentElement.classList.add('active');
                    }
                    const pedidosPane = document.querySelector('#box-pedidos');
                    if (pedidosPane) {
                        pedidosPane.classList.add('active', 'in');
                    }
                }""")
                await short_sleep_ms(500)
            
            # GUARD: Verificar se a aba está visível
            await page.wait_for_selector('#box-pedidos', state='visible', timeout=5000)
            log("[PEDIDOS][RPA] ✅ Aba Pedidos visível")
            
            # Aguardar conteúdo da aba carregar (tabela ou dropdown)
            await short_sleep_ms(1500)
            
            # Verificar se o dropdown de ações está visível (usa classe .btn-acoes.dropdown-toggle)
            dropdown_visible = await page.locator('#box-pedidos .btn-group button.btn-acoes.dropdown-toggle').first.is_visible()
            if not dropdown_visible:
                log("[PEDIDOS][RPA] Dropdown não visível - tentando via JavaScript...")
                await page.evaluate("""() => {
                    // Forçar ativação da aba Pedidos via Bootstrap
                    const tab = document.querySelector('a[href="#box-pedidos"]');
                    if (tab) tab.click();
                    
                    // Remover classes de outras abas e adicionar na aba de pedidos
                    document.querySelectorAll('.nav-tabs li').forEach(li => li.classList.remove('active'));
                    document.querySelectorAll('.tab-content .tab-pane').forEach(pane => pane.classList.remove('active', 'in'));
                    
                    const pedidosTab = document.querySelector('a[href="#box-pedidos"]');
                    if (pedidosTab && pedidosTab.parentElement) {
                        pedidosTab.parentElement.classList.add('active');
                    }
                    const pedidosPane = document.querySelector('#box-pedidos');
                    if (pedidosPane) {
                        pedidosPane.classList.add('active', 'in');
                        pedidosPane.style.display = 'block';
                    }
                }""")
                await short_sleep_ms(1000)
        ...
        # Loop para adicionar cada pedido
            ...
            try:
                ...
                for existing in existing_pedidos:
                    if tipo_norm == existing or tipo_norm in existing or existing in tipo_norm:
                        log(f"[PEDIDOS][RPA][SKIP] Pedido '{tipo_text}' já existe (similar a '{existing}')")
                        is_duplicate = True
                        skipped_count += 1
                        break
                ...
                log(f"[PEDIDOS][RPA] Adicionando pedido {idx + 1}/{len(tipos_pedidos)}: {tipo_text}")
                update_status("pedidos_adicionando", f"Adicionando pedido {idx + 1}/{len(tipos_pedidos)}...", process_id=process_id)
                
                # ✅ FORÇAR FECHAMENTO DE QUALQUER MODAL ABERTO ANTES DE CONTINUAR
                for close_attempt in range(3):
                    try:
                        modal_visible = await page.locator('.modal.in, .modal.show, #dialog-modal.in').first.is_visible()
                        if modal_visible:
                            log(f"[PEDIDOS][RPA] Modal bloqueando - fechando via JavaScript (tentativa {close_attempt + 1})...")
                            await page.evaluate("""() => {
                                // Fechar via botão close
                                document.querySelectorAll('.modal.in button.close, .modal.show button.close, #dialog-modal button.close').forEach(btn => btn.click());
                                // Remover classe 'in' dos modals
                                document.querySelectorAll('.modal.in').forEach(m => m.classList.remove('in', 'show'));
                                // Remover backdrop
                                document.querySelectorAll('.modal-backdrop').forEach(el => el.remove());
                                // Forçar hide
                                document.querySelectorAll('#dialog-modal').forEach(m => { m.style.display = 'none'; m.classList.remove('in', 'show'); });
                            }""")
                            await short_sleep_ms(800)
                        else:
                            break
                ...
                try:
                    await page.wait_for_selector('.modal.in, .modal.show', state='hidden', timeout=2000)
                except:
                    pass
                
                # 1.5 GARANTIR que a aba Pedidos está ativa (pode ter mudado após fechar modal)
                try:
                    aba_pedidos = page.locator('a[href="#box-pedidos"]')
                    await aba_pedidos.click(timeout=3000)
                    await short_sleep_ms(500)
                    await page.wait_for_selector('#box-pedidos', state='visible', timeout=3000)
                except:
                ...
                # 2. Abrir dropdown "Ações" (usar seletor atualizado)
                log("[PEDIDOS][RPA] Abrindo dropdown Ações...")
                acoes_dropdown = page.locator('#box-pedidos .btn-group button.btn-acoes.dropdown-toggle').first
                await acoes_dropdown.click(timeout=5000)
                await short_sleep_ms(500)
                ...
                # ✅ AGUARDAR AJAX: Garantir que opções do select carregaram
                log("[PEDIDOS][RPA] Aguardando carregamento do select de tipos...")
                try:
                    await page.wait_for_function(
                        """() => {
                    ...
                    log("[PEDIDOS][RPA][WARN] Timeout aguardando opções do select")
                    await short_sleep_ms(1500)  # Delay extra
                
                # ✅ VERIFICAR se o value existe no select antes de tentar selecionar
                ...
                if not value_exists:
                    log(f"[PEDIDOS][RPA][SKIP] Tipo '{tipo_text}' (value={tipo_value}) não encontrado no select - pulando")
                    # Fechar modal e continuar
                    await page.evaluate("document.querySelector('#dialog-modal button.close')?.click()")
                    await short_sleep_ms(500)
                    continue
                
                # 5. Selecionar Tipo do Pedido
                tipo_select = page.locator('#TipoPedidoId')
                await tipo_select.select_option(value=str(tipo_value))
                await short_sleep_ms(500)
                
                # Verificar se realmente selecionou
                selected_value = await tipo_select.input_value()
                if not selected_value or selected_value == "":
                    log(f"[PEDIDOS][RPA][WARN] Tipo '{tipo_text}' não foi selecionado corretamente - pulando")
                    await page.evaluate("document.querySelector('#dialog-modal button.close')?.click()")
                    await short_sleep_ms(500)
                    continue
                
                log(f"[PEDIDOS][RPA] Tipo selecionado: {tipo_text} (value={selected_value})")
                
                # 6. Clicar no campo "Valor Pedido" para preencher 0,00
                log("[PEDIDOS][RPA] Preenchendo Valor Pedido com 0,00...")
                valor_input = page.locator('#Valor')
                await valor_input.click()
                await short_sleep_ms(300)
                # O campo money pode auto-preencher com 0,00 ao clicar
                # Se não, preencher manualmente
                current_value = await valor_input.input_value()
                if not current_value or current_value.strip() == "":
                    await valor_input.fill("0,00")
                    await short_sleep_ms(200)
                
                # Disparar eventos para campos money
                await valor_input.evaluate("""el => {
                    ['input', 'change', 'blur', 'keyup'].forEach(ev => {
                        try { el.dispatchEvent(new Event(ev, {bubbles: true})); } catch(e) {}
                    });
                }""")
                
                # 7. Clicar em Salvar (usar .first para evitar ambiguidade com "Salvar e Fechar")
                log("[PEDIDOS][RPA] Clicando em Salvar...")
                save_btn = page.locator('.modal button.btn-primary.salvar:not(.salvarfechar)').first
                await save_btn.click()
                await short_sleep_ms(2000)
                
                # 8. Aguardar modal fechar (se não fechar, houve erro de validação)
                try:
                    await page.wait_for_selector('.modal.in, .modal.show', state='hidden', timeout=5000)
                    log(f"[PEDIDOS][RPA] ✅ Pedido {idx + 1} adicionado com sucesso!")
                    success_count += 1
                    existing_pedidos.add(tipo_norm)
                except:
                    # Modal não fechou = erro de validação (tipo não selecionado, valor vazio, etc)
                    log(f"[PEDIDOS][RPA][ERRO] Modal não fechou após pedido {idx + 1} - verificando erro de validação...")
                    
                    # Verificar se há mensagem de erro visível
                    try:
                        error_msg = await page.locator('.modal .validation-summary-errors, .modal .field-validation-error, .modal .alert-danger').first.text_content()
                        if error_msg:
                            log(f"[PEDIDOS][RPA][ERRO] Erro de validação: {error_msg.strip()[:100]}")
                    except:
                        pass
                    
                    # Fechar modal manualmente via JavaScript
                    log("[PEDIDOS][RPA] Fechando modal manualmente...")
                    await page.evaluate("""() => {
                        document.querySelectorAll('#dialog-modal button.close, .modal.in button.close').forEach(btn => btn.click());
                        document.querySelectorAll('.modal.in, .modal.show').forEach(m => { m.classList.remove('in', 'show'); m.style.display = 'none'; });
                        document.querySelectorAll('.modal-backdrop').forEach(el => el.remove());
                    }""")
                    await short_sleep_ms(800)
                    
                    # NÃO contar como sucesso - pedido não foi salvo
                    log(f"[PEDIDOS][RPA][WARN] Pedido {idx + 1} NÃO foi salvo - pulando")
                
            except Exception as e:
                log(f"[PEDIDOS][RPA][ERRO] Falha ao adicionar pedido {idx + 1}: {e}")
                # Tentar fechar modal se estiver aberto via JavaScript
                try:
                    await page.evaluate("document.querySelector('#dialog-modal')?.querySelector('button.close')?.click()")
                    await short_sleep_ms(500)
                except:
        ...
        log(f"[PEDIDOS][RPA] Adicionados {success_count}/{len(tipos_pedidos)} pedidos (pulados: {skipped_count} duplicatas)")
...
async def _map_pedidos_to_tipos(page, pedidos_list: list, objeto: str) -> list:
    ...
    """
    # Palavras-chave para matching de pedidos trabalhistas comuns
    PEDIDO_KEYWORDS = {
        "horas extras": ["hora extra", "horas extras", "sobrejornada", "adicional noturno", "jornada extraordinária"],
        "verbas rescisórias": ["verbas rescisórias", "rescisão", "aviso prévio", "multa 40%", "fgts", "saldo de salário"],
        "danos morais": ["dano moral", "danos morais", "indenização moral", "assédio moral"],
        "danos materiais": ["dano material", "danos materiais", "indenização material"],
        "vínculo empregatício": ["vínculo", "reconhecimento", "anotação ctps", "registro emprego"],
        "adicional insalubridade": ["insalubridade", "insalubre", "adicional de insalubridade"],
        "adicional periculosidade": ["periculosidade", "periculoso", "adicional de periculosidade"],
        "diferenças salariais": ["diferença salarial", "diferenças salariais", "equiparação salarial"],
        "férias": ["férias", "terço de férias", "férias proporcionais", "férias vencidas"],
        "13º salário": ["13º salário", "décimo terceiro", "gratificação natalina"],
        "multa art 477": ["multa 477", "art. 477", "artigo 477", "atraso rescisão"],
        "multa art 467": ["multa 467", "art. 467", "artigo 467", "verbas incontroversas"],
        "honorários": ["honorários", "honorários advocatícios", "sucumbência"],
        "justiça gratuita": ["justiça gratuita", "gratuidade", "assistência judiciária"],
    }
    ...
    # Buscar opções do select
    try:
        # Primeiro, abrir o modal para ter acesso ao select
        acoes_dropdown = page.locator('#box-pedidos .btn-group button.btn-acoes.dropdown-toggle').first
        log("[PEDIDOS][MAP] Clicando no dropdown Ações...")
        await acoes_dropdown.click(timeout=5000)  # Timeout curto para evitar esperar 30s
        await short_sleep_ms(500)
        
        novo_pedido = page.locator('#buttonNewPedido')
        log("[PEDIDOS][MAP] Clicando em Novo Pedido...")
        await novo_pedido.click(timeout=5000)
        await short_sleep_ms(1000)
        ...
        try:
            ...
            log("[PEDIDOS][MAP] Select de tipos carregado com opções")
        except:
            log("[PEDIDOS][MAP] Aguardando opções do select (timeout)...")
            await short_sleep_ms(1000)
        ...
        for option in options:
            ...
            # Match por keywords
            for keyword_group, keywords in PEDIDO_KEYWORDS.items():
                if any(kw in texto_busca_lower for kw in keywords):
                    if any(kw in option_lower for kw in keywords):
                        if option_value not in tipos_values_usados:
                            ...
                            log(f"[PEDIDOS][MAP] ✅ Match: {keyword_group} -> {option_text}")
        
        # Fechar modal após mapear (usar .first para evitar strict mode)
        try:
            close_btn = page.locator('#dialog-modal button.close, .modal.in button.close').first
            await close_btn.click(timeout=3000)
            await short_sleep_ms(500)
        ...
        # Tentar fechar modal via JavaScript
        try:
            await page.evaluate("document.querySelector('#dialog-modal')?.querySelector('button.close')?.click()")
            await short_sleep_ms(300)
        except:
...
# =========================
async def ensure_on_new_process_form(page):
    """
    ...
    for dashboard_attempt in range(max_dashboard_attempts):
        try:
            ...
            if dashboard_attempt == max_dashboard_attempts - 1:
                ...
                raise RuntimeError(f"Falha ao garantir sessão logada após {max_dashboard_attempts} tentativas: {e}")
    ...
    target = BASE_URL.rstrip("/") + "/Processo/form"
    log(f"[BATCH] Navegando para: {target}")
    
    try:
        await goto_with_retries(page, target, attempts=2, nav_timeout_ms=15000)
        await page.wait_for_url(re.compile(form_url_pattern), timeout=10000)
...
async def after_login_flow(page, process_id: int):
    ...
    await ensure_on_new_process_form(page)
