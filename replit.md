...
*   **Suporte a PDFs Grandes (2025-12-05):** Limite de upload aumentado de 16MB para 100MB por arquivo. Limite total de request 2GB para suportar batches grandes. Front-end atualizado para exibir limite correto.
...
*   **Múltiplas Reclamadas Inteligente (2025-12-05):** Sistema de detecção de clientes cadastrados para seleção correta de posição:
    - Verifica se reclamada extra é cliente cadastrado no dicionário usando `find_cliente_by_parte_interessada()`
    - Se for cliente cadastrado (ex: CSN como 2ª reclamada): seleciona "OUTROS CLIENTES RÉU" no dropdown
    - Se NÃO for cliente (ex: SOLAZER TRANSPORTE): mantém comportamento normal "OUTRA PARTE" ou "RECLAMADO"
    - Correção de bug de repetição: verifica se reclamada extra é diferente do cliente principal antes de inserir
    - Mapeamento atualizado: POSICAO_MAP inclui valores 71 (OUTROS CLIENTES RÉU), 39 (OUTRA PARTE)
...
*   **Ordenação por Tamanho (2025-12-05):** PDFs menores são processados primeiro durante extração em batch:
    - Campo `file_size` (BigInteger) adicionado ao BatchItem para armazenar tamanho em bytes
