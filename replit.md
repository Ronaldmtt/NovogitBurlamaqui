...
*   **OCR Seletivo via Bookmarks (2025-12-05):** PDFs do PJe contêm anexos (TRCT, Contracheques, Ficha CTPS) como imagens embutidas que requerem OCR. Nova estratégia ultra-otimizada:
    - `extract_pdf_bookmarks()`: Extrai mapeamento EXATO dos bookmarks do PDF (CTPS→pág.19, TRCT→pág.21, Contracheque→pág.29)
    - OCR aplicado apenas nas páginas identificadas pelos bookmarks (1-2 páginas ao invés de 5)
    - Fallback hierárquico: Bookmarks → Sumário textual → Heurística de páginas escaneadas
    - Economia de ~90 segundos por PDF (60% mais rápido)
    - Instalado Tesseract com 129 idiomas (incluindo português)
