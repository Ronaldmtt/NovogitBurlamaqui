...
*   **OCR para Anexos PJe (2025-12-05):** PDFs do PJe contêm anexos (TRCT, Contracheques, Ficha CTPS) como imagens embutidas que requerem OCR. Instalado Tesseract com 129 idiomas (incluindo português). Função `detect_scanned_pages()` melhorada para detectar todas as páginas de imagem (< 200 chars de texto nativo), não apenas as últimas 30%. Dados extraíveis via OCR: Data Admissão, PIS, CTPS, Série CT, Salário Base.
